...
def average_color_of_quadrilateral(image, points):
    ...
    # Draw the quadrilateral on the mask and fill it
    points = np.array(points, dtype='int32')
    cv2.fillPoly(mask, [points], (255))
    
    # Convert mask to boolean for masking
    mask = mask.astype(bool)
    
    # Use the mask to select the region of the image inside the quadrilateral
    selected_region = image[mask]
...
# average_color = average_color_of_quadrilateral(image_path, points)
    ...
    if success:
        ...
        for person, box in zip(results[0].keypoints.xy, results[0].boxes.xyxy):
            ...
            if len(coords) == 4: 
                ...
                average_colors.append((average_color_of_quadrilateral(frame, coords), coords[0]))
                coords = np.array(coords, dtype='int32')
                cv2.fillPoly(annotated_frame, pts=[coords], color=average_colors[-1][0])
            else: 
                for coord in coords: 
